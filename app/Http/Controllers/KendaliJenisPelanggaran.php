...
class KendaliJenisPelanggaran extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $query = DB::table('jenis_pelanggarans')
            ->select(
                ...
                'kategori_pelanggarans.id as kategori_id',
            ...
            ->when($filterKategori, function ($query) use ($filterKategori) {
                return $query->where('kategori_pelanggarans.id', $filterKategori);
            })
        ...
        // Get statistics - count by category name (allows same name with different points)
        $stats = [
            'total_jenis' => DB::table('jenis_pelanggarans')->count(),
            'ringan' => DB::table('jenis_pelanggarans')
                ->join('kategori_pelanggarans', 'jenis_pelanggarans.id_kategori_pelanggaran', '=', 'kategori_pelanggarans.id')
                ->where('kategori_pelanggarans.nama', 'like', 'Ringan')
                ->count(),
            'sedang' => DB::table('jenis_pelanggarans')
                ->join('kategori_pelanggarans', 'jenis_pelanggarans.id_kategori_pelanggaran', '=', 'kategori_pelanggarans.id')
                ->where('kategori_pelanggarans.nama', 'like', 'Sedang')
                ->count(),
            'berat' => DB::table('jenis_pelanggarans')
                ->join('kategori_pelanggarans', 'jenis_pelanggarans.id_kategori_pelanggaran', '=', 'kategori_pelanggarans.id')
                ->where('kategori_pelanggarans.nama', 'like', 'Berat')
                ->count(),
        ];
    ...
    public function storeKategori(Request $request)
    {
        ...
        // Allow same name with different points - validate unique combination of nama + poin
        $request->validate([
            'nama' => 'required|string|max:50',
            'poin' => 'required|integer|min:1',
        ]);

        // Check if combination of nama and poin already exists
        $exists = DB::table('kategori_pelanggarans')
            ->where('nama', $request->nama)
            ->where('poin', $request->poin)
            ->exists();

        if ($exists) {
            return redirect()->back()
                ->with('error', 'Kategori "' . $request->nama . '" dengan poin ' . $request->poin . ' sudah ada.')
                ->withInput();
        }
    ...
    public function updateKategori(Request $request, $id)
    {
        ...
        // Check if combination of nama and poin already exists (excluding current record)
        $exists = DB::table('kategori_pelanggarans')
            ->where('nama', $request->nama)
            ->where('poin', $request->poin)
            ->where('id', '!=', $id)
