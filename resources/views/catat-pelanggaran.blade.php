        ...
        <main class="flex-1 max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-8">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div class="bg-white rounded-2xl p-6 shadow-lg shadow-gray-200/50 border border-gray-100 animate-fade-in delay-100">
                    ...
                    <!-- Search Tab -->
                    <div id="search-tab">
                        <form action="{{ route('pelanggaran.catat') }}" method="GET" class="mb-4">
                            <div class="relative">
                                <input type="text"
                                    name="search_siswa"
                                    id="search-input"
                        ...
                        @if(count($siswaResult) > 0)
                            <div class="space-y-2">
                                @foreach($siswaResult as $s)
                                    <a href="{{ route('pelanggaran.catat', ['id_siswa' => $s->id]) }}"
                                       class="flex items-center gap-4 p-3 rounded-xl hover:bg-gray-50 transition-colors border border-gray-100">
                                        ...
                                        <div class="flex-1">
                                            ...
                                            <p class="text-sm text-gray-500">{{ $s->nis }} - {{ $s->kelas }}</p>
                                        </div>
                        ...
                        @elseif($searchSiswa && count($siswaResult) == 0)
                ...
                <div class="bg-white rounded-2xl p-6 shadow-lg shadow-gray-200/50 border border-gray-100 animate-fade-in delay-200">
                    @if($selectedSiswa)
                        ...
                        <div class="flex items-center gap-4 p-4 bg-indigo-50 rounded-xl mb-6 border border-indigo-100">
                            ...
                            <div class="flex-1">
                                <p class="font-bold text-gray-800 text-lg">{{ $selectedSiswa->name }}</p>
                                <p class="text-sm text-gray-600">{{ $selectedSiswa->nis }} - {{ $selectedSiswa->kelas }}</p>
                                <p class="text-sm mt-1">
                                    <span class="font-medium">Total Poin:</span>
                                    <span class="{{ $selectedSiswa->total_poin > 10 ? 'text-rose-600' : ($selectedSiswa->total_poin > 0 ? 'text-amber-600' : 'text-emerald-600') }}">
                                        {{ $selectedSiswa->total_poin }} Poin
                                    </span>
                                </p>
                            </div>
                            <a href="{{ route('pelanggaran.catat') }}" class="p-2 text-gray-400 hover:text-gray-600">
                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        <!-- Violation Form -->
                        <form action="{{ route('pelanggaran.catat.action') }}" method="POST" enctype="multipart/form-data">
                            @csrf
                            ...
                            <div class="mb-4">
                                ...
                                <select name="id_jenis_pelanggaran" id="jenis_pelanggaran" required class="w-full px-4 py-3 rounded-xl border border-gray-200 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-200 transition-all outline-none bg-white" onchange="checkCategory()">
                                    ...
                                    @foreach($kategoriList as $kategori => $pelanggarans)
                                        <optgroup label="{{ $kategori }}" data-category="{{ $kategori }}">
                                            @foreach($pelanggarans as $p)
                                                <option value="{{ $p->id }}" data-category="{{ $kategori }}">{{ $p->nama }} ({{ $p->poin }} poin)</option>
                                            @endforeach
                                        </optgroup>
                                    @endforeach
                                </select>
