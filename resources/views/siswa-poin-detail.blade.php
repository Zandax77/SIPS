...
<body class="bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-100 min-h-screen">
        ...
        <main class="flex-1 max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-8">
            ...
            <div class="bg-white rounded-2xl p-6 shadow-lg shadow-gray-200/50 border border-gray-100 mb-6 animate-fade-in delay-100">
                <div class="flex flex-col md:flex-row md:items-center gap-6">
                    ...
                    <div class="flex flex-col items-end">
                        ...
                        <p class="text-4xl font-bold {{ $poinEfektif > 10 ? 'text-rose-600' : ($poinEfektif > 0 ? 'text-amber-600' : 'text-emerald-600') }}">
                            {{ $poinEfektif }}
                        </p>
                        <p class="text-sm text-gray-400">poin</p>
                        @if($totalPenguranganPoin > 0)
                            <p class="text-xs text-emerald-600 mt-1">({{ $totalPoin }} - {{ $totalPenguranganPoin }} poin prestasi)</p>
                        @endif
                    </div>
                </div>
            </div>
                        ...
                        ">
                            <div class="flex items-start justify-between">
                                <div class="flex-1">
                                    ...
                                    <div class="mt-2">
                                        <button onclick="showTindakanEvidence('{{ $tindakan->bukti_foto }}')" class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium bg-purple-50 text-purple-700 hover:bg-purple-100 transition-colors">
                                ...
                                <div class="flex items-center gap-2">
                                    @if(session('jabatan') !== 'Kepala Sekolah')
                                    <button onclick="editTindakan({{ $tindakan->id }}, '{{ $tindakan->jenis_tindakan }}', '{{ $tindakan->deskripsi_tindakan }}', '{{ $tindakan->hasil_tindakan }}', '{{ $tindakan->catatan_hasil }}', '{{ $tindakan->tanggal_tindakan }}', '{{ $tindakan->bukti_foto ?? '' }}')" class="p-2 text-gray-400 hover:text-indigo-600 hover:bg-indigo-50 rounded-lg transition-colors" title="Edit">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div class="bg-white rounded-2xl p-6 shadow-lg shadow-gray-200/50 border border-emerald-100 mb-6 animate-fade-in">
                <div class="flex items-center justify-between mb-6">
                    ...
                    @if(session('jabatan') !== 'Kepala Sekolah')
                    <a href="{{ route('prestasi.create', ['siswa_id' => $siswa->id]) }}" class="px-4 py-2 bg-emerald-600 text-white rounded-lg hover:bg-emerald-700 transition-colors font-medium flex items-center gap-2">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"></path>
                        </svg>
                        Tambah Prestasi
                ...
                @if(isset($prestasiSiswa) && count($prestasiSiswa) > 0)
                    <div class="space-y-4">
                            ...
                            <div class="flex items-start justify-between">
                                <div class="flex-1">
                                    <div class="flex items-center gap-2 mb-2 flex-wrap">
                                        <span class="font-semibold text-gray-800">{{ $prestasi->nama_prestasi }}</span>
                                        @if($prestasi->bidang == 'Akademik')
                                        ...
                                        @endphp
                                        <span class="px-2 py-0.5 rounded-full text-xs font-medium {{ $tColor }}">{{ $prestasi->tingkat }}</span>
                                    </div>
                                    @if($prestasi->peringkat)
                                        <p class="text-sm text-gray-700"><span class="font-medium">Peringkat:</span> {{ $prestasi->peringkat }}</p>
                                    @endif
                                    @if($prestasi->deskripsi)
                                        <p class="text-sm text-gray-600 mt-1">{{ $prestasi->deskripsi }}</p>
                                    @endif
                                    @if($prestasi->pengurangan_poin > 0)
                                        <div class="mt-2 inline-flex items-center gap-1 px-3 py-1 rounded-lg bg-emerald-100 text-emerald-700 text-sm font-medium">
                                            ...
                                            Pengurangan Poin: {{ $prestasi->pengurangan_poin }}
                                        </div>
                                    @endif
                                    @if($prestasi->bukti_foto)
                                        <div class="mt-2">
                                            ...
                                            @endphp
                                            @if($isImage)
                                                <img src="{{ asset('storage/' . $prestasi->bukti_foto) }}" alt="Bukti Prestasi" class="w-32 h-24 object-cover rounded-lg border border-gray-200 mt-2">
                                            @else
                                                <a href="{{ asset('storage/' . $prestasi->bukti_foto) }}" target="_blank" class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium bg-emerald-50 text-emerald-700 hover:bg-emerald-100 transition-colors">
                                                    <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    ...
                                    @endif
                                    <p class="text-xs text-gray-400 mt-2">{{ \Carbon\Carbon::parse($prestasi->tanggal_prestasi)->format('d/m/Y') }}</p>
                                </div>
                            </div>
                        </div>
                        @endforeach
                ...
                @else
                    <div class="text-center py-8 bg-gray-50 rounded-xl">
                        ...
                        <a href="{{ route('prestasi.create', ['siswa_id' => $siswa->id]) }}" class="mt-2 text-sm text-emerald-600 hover:text-emerald-700 font-medium inline-flex items-center gap-1">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"></path>
                            </svg>
                            Catat prestasi baru
    ...
    <div id="tindakanModal" class="fixed inset-0 z-50 hidden">
        ...
        <div class="absolute inset-4 md:inset-10 bg-white rounded-2xl shadow-2xl overflow-hidden flex flex-col">
            ...
            <!-- Modal Body -->
            <div class="flex-1 overflow-y-auto p-6">
                <form id="tindakan-form" action="{{ route('siswa.tindakan.store', ['id' => $siswa->id]) }}" method="POST" enctype="multipart/form-data">
                    @csrf
