    ...
    <div class="relative min-h-screen flex flex-col">
        ...
        <main class="flex-1 max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-8">
            ...
            <div class="bg-white rounded-2xl p-6 shadow-lg shadow-gray-200/50 border border-gray-100 mb-6 animate-fade-in">
                <form action="{{ route('laporan.siswa-tertinggi') }}" method="GET" class="flex flex-col sm:flex-row gap-4">
                ...
                <div class="mt-4 pt-4 border-t border-gray-100 flex flex-wrap gap-3">
                    <a href="{{ route('laporan.siswa-tertinggi.pdf', ['kelas' => $kelas, 'limit' => $limit]) }}" target="_blank" class="inline-flex items-center px-4 py-2 bg-rose-100 text-rose-700 rounded-lg hover:bg-rose-200 transition-colors font-medium">
                        <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 21h10a2 2 0 002-2V9.414a1 1 0 00-.293-.707l-5.414-5.414A1 1 0 0012.586 3H7a2 2 0 00-2 2v14a2 2 0 002 2z"></path>
                        </svg>
                        Export PDF
                    </a>
                    <a href="{{ route('laporan.siswa-tertinggi.excel', ['kelas' => $kelas, 'limit' => $limit]) }}" class="inline-flex items-center px-4 py-2 bg-emerald-100 text-emerald-700 rounded-lg hover:bg-emerald-200 transition-colors font-medium">
                        <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                ...
                <div class="bg-white rounded-2xl p-6 shadow-lg shadow-gray-200/50 border border-gray-100 order-2 md:order-1 animate-fade-in">
                    <div class="text-center">
                        ...
                        <p class="text-lg font-semibold text-gray-800">{{ $siswa[1]->nama_siswa }}</p>
                        <p class="text-sm text-gray-500">{{ $siswa[1]->kelas }}</p>
                        <div class="mt-4 inline-flex items-center px-4 py-2 bg-amber-100 text-amber-700 rounded-full">
                            <span class="text-xl font-bold">{{ $siswa[1]->total_poin }}</span>
                            <span class="ml-1 text-sm">poin</span>
                        </div>
                        <p class="text-xs text-gray-400 mt-2">{{ $siswa[1]->total_pelanggaran }} pelanggaran</p>
                    </div>
                </div>
                ...
                <div class="bg-gradient-to-br from-yellow-50 to-amber-100 rounded-2xl p-6 shadow-xl shadow-yellow-200/50 border-2 border-yellow-300 order-1 md:order-2 animate-fade-in">
                    <div class="text-center">
                        ...
                        <p class="text-xl font-bold text-gray-800">{{ $siswa[0]->nama_siswa }}</p>
                        <p class="text-sm text-gray-600">{{ $siswa[0]->kelas }}</p>
                        <div class="mt-4 inline-flex items-center px-6 py-3 bg-yellow-400 text-white rounded-full shadow-md">
                            <span class="text-2xl font-bold">{{ $siswa[0]->total_poin }}</span>
                            <span class="ml-1 text-sm">poin</span>
                        </div>
                        <p class="text-xs text-gray-500 mt-2">{{ $siswa[0]->total_pelanggaran }} pelanggaran</p>
                    </div>
                </div>
                ...
                <div class="bg-white rounded-2xl p-6 shadow-lg shadow-gray-200/50 border border-gray-100 order-3 animate-fade-in">
                    <div class="text-center">
                        ...
                        <p class="text-lg font-semibold text-gray-800">{{ $siswa[2]->nama_siswa }}</p>
                        <p class="text-sm text-gray-500">{{ $siswa[2]->kelas }}</p>
                        <div class="mt-4 inline-flex items-center px-4 py-2 bg-amber-100 text-amber-700 rounded-full">
                            <span class="text-xl font-bold">{{ $siswa[2]->total_poin }}</span>
                            <span class="ml-1 text-sm">poin</span>
                        </div>
                        <p class="text-xs text-gray-400 mt-2">{{ $siswa[2]->total_pelanggaran }} pelanggaran</p>
                    </div>
                </div>
            </div>
            @endif
                ...
                <div class="px-6 py-4 border-b border-gray-100 bg-gradient-to-r from-indigo-50 to-white">
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p class="text-sm text-gray-500">Top {{ $limit }} Siswa</p>
                        </div>
                    </div>
                </div>
                        ...
                        <table class="w-full">
                            ...
                            <tbody class="divide-y divide-gray-100">
                                @foreach($siswa as $index => $data)
                                    <tr class="hover:bg-gray-50 transition-colors duration-150 {{ $index < 3 ? 'bg-yellow-50/50' : '' }}">
                                        <td class="px-6 py-4 whitespace-nowrap text-center">
                                            ...
                                            @else
                                                <span class="inline-flex items-center justify-center w-8 h-8 rounded-full bg-gray-100 text-gray-600 font-semibold">{{ $data->rank }}</span>
                                            @endif
                                        </td>
