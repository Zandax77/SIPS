            ...
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <div class="flex justify-between items-center h-16">
                    <div class="flex items-center gap-3">
                        <button onclick="window.location.href='{{ route('dashboard') }}'" class="p-2 text-gray-600 hover:text-indigo-600 hover:bg-indigo-50 rounded-lg transition-colors">
        ...
        <aside class="fixed inset-y-0 left-0 z-40 w-64 bg-white border-r border-gray-100 lg:h-screen lg:overflow-y-auto lg:block flex-shrink-0 hidden lg:block">
            ...
            <div class="flex flex-col h-full">
                <nav class="flex-1 px-4 py-6 space-y-2">
                    ...
                    <a href="{{ route('pelanggaran.laporan.kelas') }}" class="flex items-center gap-3 px-4 py-3 rounded-xl bg-indigo-50 text-indigo-600 border border-indigo-100">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 17v-2m3 2v-4m3 4v-6m2 10H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"></path>
                        </svg>
                    ...
                    <a href="{{ route('admin.petugas.index') }}" class="flex items-center gap-3 px-4 py-3 rounded-xl text-gray-600 hover:bg-gray-indigo-600-50 hover:text transition-colors">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    @if(($role ?? '') === 'admin')
                    <a href="{{ route('admin.sekolah.index') }}" class="flex items-center gap-3 px-4 py-3 rounded-xl text-gray-600 hover:bg-gray-50 hover:text-indigo-600 transition-colors">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="flex-1 lg:ml-64">
            <main class="flex-1 max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-8">
                ...
                <div class="bg-white rounded-2xl p-6 shadow-lg shadow-gray-200/50 border border-gray-100 mb-6 animate-fade-in">
                    <form action="{{ route('pelanggaran.laporan.kelas') }}" method="GET" class="space-y-4">
                        ...
                        <div class="flex flex-wrap gap-3 pt-2">
                            ...
                            @if($selectedKelas)
                            <a href="{{ route('pelanggaran.laporan.kelas.cetak', ['kelas' => $selectedKelas, 'tanggal_mulai' => $tanggalMulai, 'tanggal_akhir' => $tanggalAkhir]) }}" target="_blank" class="px-6 py-2 bg-emerald-600 text-white font-medium rounded-lg hover:bg-emerald-700 focus:ring-4 focus:ring-emerald-200 transition-colors">
                                <svg class="w-5 h-5 inline-block mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div class="space-y-6">
                        ...
                        <div class="bg-white rounded-xl p-5 shadow-lg shadow-gray-200/50 border border-gray-100">
                            <div class="flex items-center justify-between">
                                <div>
                                    ...
                                    <p class="text-3xl font-bold text-gray-800 mt-1">{{ count($rekapPerSiswa) }}</p>
                                </div>
                        ...
                        <div class="bg-white rounded-xl p-5 shadow-lg shadow-gray-200/50 border border-gray-100">
                            <div class="flex items-center justify-between">
                                <div>
                                    ...
                                    <p class="text-3xl font-bold text-gray-800 mt-1">{{ $selectedKelas }}</p>
                                </div>
                    ...
                    <div class="bg-white rounded-2xl shadow-lg shadow-gray-200/50 border border-gray-100 overflow-hidden">
                        <div class="px-6 py-4 border-b border-gray-100 bg-gradient-to-r from-indigo-50 to-white">
                            ...
                            <p class="text-sm text-gray-500">Ringkasan pelanggaran siswa di kelas {{ $selectedKelas }}</p>
                        </div>
                                ...
                                <tbody class="divide-y divide-gray-100">
                                    @foreach($rekapPerSiswa as $index => $siswa)
                                        <tr class="hover:bg-gray-50 transition-colors">
                                            ...
                                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-800">{{ $siswa->nama_siswa }}</td>
                    ...
                    <div class="bg-white rounded-2xl shadow-lg shadow-gray-200/50 border border-gray-100 overflow-hidden">
                        <div class="px-6 py-4 border-b border-gray-100 bg-gradient-to-r from-rose-50 to-white">
                            ...
                            <p class="text-sm text-gray-500">Data lengkap pelanggaran siswa di kelas {{ $selectedKelas }}</p>
                        </div>
                                ...
                                <tbody class="divide-y divide-gray-100">
                                    @foreach($pelanggaranData as $index => $data)
                                        <tr class="hover:bg-gray-50 transition-colors">
                                            ...
                                            <td class="px-4 py-3 text-sm text-gray-600 max-w-xs truncate">{{ $data->deskripsi ?: '-' }}</td>
                                            <td class="px-4 py-3 text-sm text-gray-600">{{ $data->nama_petugas }}</td>
                                            <td class="px-4 py-3 text-sm text-gray-500 whitespace-nowrap">{{ \Carbon\Carbon::parse($data->created_at)->format('d/m/Y') }}</td>
                                        </tr>
                                    @endforeach
                                </tbody>
                            </table>
                    ...
                    <div class="bg-white rounded-2xl p-12 shadow-lg shadow-gray-200/50 border border-gray-100 text-center">
                        ...
                        <p class="mt-4 text-gray-500 text-lg">Tidak ada pelanggaran untuk kelas {{ $selectedKelas }} pada periode yang dipilih.</p>
                    </div>
                    @endif
                </div>
                @else
