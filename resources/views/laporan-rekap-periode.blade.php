    ...
    <div class="relative min-h-screen flex flex-col">
        ...
        <main class="flex-1 max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-8">
            ...
            <div class="bg-white rounded-2xl p-6 shadow-lg shadow-gray-200/50 border border-gray-100 mb-6 animate-fade-in">
                <form action="{{ route('laporan.rekap-periode') }}" method="GET" class="flex flex-col sm:flex-row gap-4">
                ...
                <div class="mt-4 pt-4 border-t border-gray-100 flex flex-wrap gap-3">
                    <a href="{{ route('laporan.rekap-periode.pdf', ['tanggal_mulai' => $tanggalMulai, 'tanggal_akhir' => $tanggalAkhir, 'kelas' => $kelas]) }}" target="_blank" class="inline-flex items-center px-4 py-2 bg-rose-100 text-rose-700 rounded-lg hover:bg-rose-200 transition-colors font-medium">
                        <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 21h10a2 2 0 002-2V9.414a1 1 0 00-.293-.707l-5.414-5.414A1 1 0 0012.586 3H7a2 2 0 00-2 2v14a2 2 0 002 2z"></path>
                        </svg>
                        Export PDF
                    </a>
                    <a href="{{ route('laporan.rekap-periode.excel', ['tanggal_mulai' => $tanggalMulai, 'tanggal_akhir' => $tanggalAkhir, 'kelas' => $kelas]) }}" class="inline-flex items-center px-4 py-2 bg-emerald-100 text-emerald-700 rounded-lg hover:bg-emerald-200 transition-colors font-medium">
                        <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div class="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4 mb-6">
                <div class="bg-white rounded-xl p-4 shadow-lg shadow-gray-200/50 border border-gray-100">
                    ...
                    <p class="text-2xl font-bold text-gray-800 mt-1">{{ $totals['total_hari'] }}</p>
                </div>
                <div class="bg-white rounded-xl p-4 shadow-lg shadow-gray-200/50 border border-gray-100">
                    <p class="text-xs font-medium text-gray-500">Siswa Pelaku</p>
                    <p class="text-2xl font-bold text-indigo-600 mt-1">{{ $totals['siswa_pelaku'] }}</p>
                </div>
                <div class="bg-white rounded-xl p-4 shadow-lg shadow-gray-200/50 border border-gray-100">
                ...
                <div class="px-6 py-4 border-b border-gray-100 bg-gradient-to-r from-indigo-50 to-white">
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p class="text-sm text-gray-500">Periode: {{ \Carbon\Carbon::parse($tanggalMulai)->format('d/m/Y') }} - {{ \Carbon\Carbon::parse($tanggalAkhir)->format('d/m/Y') }}</p>
                        </div>
                    </div>
                </div>
                        ...
                        <table class="w-full">
                            ...
                            <tbody class="divide-y divide-gray-100">
                                @foreach($rekap as $index => $data)
                                    <tr class="hover:bg-gray-50 transition-colors duration-150">
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-800">{{ \Carbon\Carbon::parse($data->tanggal)->format('d/m/Y') }}</td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-600 text-center">{{ $data->siswa_pelaku }}</td>
                            ...
                            <tfoot class="bg-gray-100">
                                <tr>
                                    ...
                                    <td class="px-6 py-3 text-center text-sm font-bold text-indigo-600">{{ $totals['siswa_pelaku'] }}</td>
