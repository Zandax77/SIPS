...
<body class="bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-100 min-h-screen">
                    ...
                    <div class="flex-1">
                        <div class="flex items-center justify-between mb-2">
                            ...
                            <span class="px-3 py-1 rounded-full text-xs font-medium
                                <?php if($aiSuggestions['priority_level'] <= 2): ?> bg-emerald-100 text-emerald-700
                                <?php elseif($aiSuggestions['priority_level'] == 3): ?> bg-amber-100 text-amber-700
                                <?php else: ?> bg-rose-100 text-rose-700 <?php endif; ?>">
                                Prioritas <?php echo e($aiSuggestions['priority_level']); ?>/5
                            </span>
                        </div>

                        <!-- Recommendation Card -->
                        <div class="bg-white rounded-xl p-4 mb-4 border-l-4
                            <?php if($aiSuggestions['recommendation']['color'] == 'emerald'): ?> border-emerald-500
                            <?php elseif($aiSuggestions['recommendation']['color'] == 'blue'): ?> border-blue-500
                            <?php elseif($aiSuggestions['recommendation']['color'] == 'amber'): ?> border-amber-500
                            <?php elseif($aiSuggestions['recommendation']['color'] == 'orange'): ?> border-orange-500
                            <?php else: ?> border-rose-500 <?php endif; ?>">
                            <h4 class="font-semibold text-gray-800"><?php echo e($aiSuggestions['recommendation']['title']); ?></h4>
                            <p class="text-sm text-gray-600 mt-1"><?php echo e($aiSuggestions['recommendation']['description']); ?></p>
                        </div>

                        <!-- Analysis Text -->
                        <div class="bg-white/60 rounded-lg p-3 mb-4">
                            <p class="text-sm text-gray-700"><?php echo nl2br(e($aiSuggestions['analysis'])); ?></p>
                        </div>

                        <!-- ========== BARU: Violation Analysis Section ========== -->
                        <?php if(isset($aiSuggestions['violation_analysis'])): ?>
                        <div class="bg-white rounded-xl p-4 mb-4 border border-indigo-100">
                            ...
                            <?php if($aiSuggestions['violation_analysis']['nama_pelanggaran_terbanyak']): ?>
                            <div class="mb-3">
                                <span class="text-xs font-medium text-gray-500">Pelanggaran Tertinggi:</span>
                                <p class="text-sm font-medium text-indigo-700">
                                    <?php echo e($aiSuggestions['violation_analysis']['nama_pelanggaran_terbanyak']); ?>

                                    (<?php echo e($aiSuggestions['violation_analysis']['jumlah_terbanyak']); ?>x)
                                </p>
                            </div>
                            <?php endif; ?>

                            <!-- Pelanggaran Berulang Warning -->
                            <?php if($aiSuggestions['violation_analysis']['pelanggaran_berulang']): ?>
                            <div class="mb-3 p-3 bg-amber-50 rounded-lg border border-amber-200">
                                ...
                                <ul class="text-xs text-amber-700 space-y-1">
                                    <?php $__currentLoopData = $aiSuggestions['violation_analysis']['detail_pelanggaran_berulang']; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $nama => $data): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                    <li>• <?php echo e($nama); ?> - <?php echo e($data['jumlah']); ?>x</li>
                                    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                                </ul>
                            </div>
                            <?php endif; ?>
                            ...
                            <?php if(count($aiSuggestions['violation_analysis']['kata_kunci_deskripsi']) > 0): ?>
                            <div>
                                <span class="text-xs font-medium text-gray-500">Kata Kunci Dominan:</span>
                                <div class="flex flex-wrap gap-1 mt-1">
                                    <?php $__currentLoopData = $aiSuggestions['violation_analysis']['kata_kunci_deskripsi']; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $keyword => $count): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                    <span class="px-2 py-0.5 bg-gray-100 text-gray-600 rounded text-xs">
                                        <?php echo e($keyword); ?> (<?php echo e($count); ?>)
                                    </span>
                                    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                                </div>
                            </div>
                        ...
                        <!-- Action Items -->
                        <?php if(count($aiSuggestions['action_items']) > 0): ?>
                        <div class="space-y-2">
                            <p class="text-sm font-medium text-gray-700">📋 Tindakan yang Direkomendasikan:</p>
                            <?php $__currentLoopData = $aiSuggestions['action_items']; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $index => $action): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            <div class="flex items-start gap-2 bg-white rounded-lg p-3 border border-gray-100">
                                <span class="flex-shrink-0 w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold
                                    <?php if($action['priority'] == 'critical'): ?> bg-rose-600 text-white
                                    <?php elseif($action['priority'] == 'high'): ?> bg-amber-500 text-white
                                    <?php elseif($action['priority'] == 'medium'): ?> bg-blue-500 text-white
                                    <?php else: ?> bg-gray-400 text-white <?php endif; ?>">
                                    <?php echo e($index + 1); ?>

                                </span>
                                <div class="flex-1">
                                    <p class="text-sm font-medium text-gray-800"><?php echo e($action['title']); ?></p>
                                    <p class="text-xs text-gray-500"><?php echo e($action['description']); ?></p>
                                </div>
                            </div>
                            <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                        </div>
                                    ...
                                    <tr class="hover:bg-gray-50 transition-colors duration-150">
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <?php if($data->lampiran): ?>
                                                <?php if($data->tipe_lampiran === 'foto'): ?>
                                                    <button onclick="viewAttachment(<?php echo e($data->id); ?>, 'foto')"
                                                ...
                                                <?php else: ?>
                                                    <a href="<?php echo e(route('siswa.pelanggaran.download', $data->id)); ?>"
                                            ...
                                            <?php elseif($data->poin_dikurangi > 0): ?>
                                                <div class="flex items-center gap-2">
                                                    <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-amber-100 text-amber-700">
                                                        -<?php echo e($data->poin_dikurangi); ?> Poin
                                                    </span>
                                                </div>
                                            <?php endif; ?>
                                            <button onclick="showTindakanModal(<?php echo e($data->id); ?>)"
    ...
    <script>
        ...
        const pelanggaranData = <?php echo json_encode($pelanggaranDetail, 15, 512) ?>;
                ...
                <div class="space-y-4">
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <p class="text-gray-800 font-medium"><?php echo e($siswa->name); ?></p>
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-500">NIS</label>
                            <p class="text-gray-800"><?php echo e($siswa->nis); ?></p>
                        </div>
                    </div>
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            <label class="block text-sm font-medium text-gray-500">Kelas</label>
                            <p class="text-gray-800"><?php echo e($siswa->kelas); ?></p>
                        </div>
                        <div>
                    ...
                    if (p.sudah_diampuni) {
                        ...
                        actionButtons = `
                            ...
                            <div id="form-pengampunan">
                                <form action="<?php echo e(route('pelanggaran.pengampunan')); ?>" method="POST">
                                    <?php echo csrf_field(); ?>
                            ...
                            <div id="form-pengurangan" class="hidden">
                                <form action="<?php echo e(route('pelanggaran.pengurangan-poin')); ?>" method="POST">
                                    <?php echo csrf_field(); ?>
                                    <input type="hidden" name="id_pelanggaran" value="${p.id}">
